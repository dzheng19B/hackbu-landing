...
| k | Srcset triple agreement | **PASS** *(re-verified 2026-09-03 after the artwork swap to the 1672×941 16:9 aerial illustration)* | `src/lib/images.ts:29` and `scripts/generate-images.mjs:97` both `[640, 960, 1280, 1672, 2508, 3344, 5016, 6688]`; `index.html`'s `imagesrcset` carries the same eight rungs; the only artwork `sizes` is `((pointer: coarse) and (max-width: 767px)) 1114px, ((pointer: coarse) and (max-height: 500px)) 1114px, (min-aspect-ratio: 1672/941) 300vw, 533.05vh`, identical between the preload and the `<picture>`; headless Chromium selected `Campus-6688.avif` at 1900×912@1x and 1440×900@2x and — with touch emulation, where `(pointer: coarse)` matches — `Campus-3344.avif` at 390×844@3x and 844×390@3x, one campus fetch each |
